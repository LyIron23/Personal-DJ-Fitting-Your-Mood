            ...
            for item in forecast:
                ...
                if date not in dates:
                    ...
                    if 'rain' in item:
                        ...
                        rains.append(0)
            # a bar graph to display temperature n rain
...
def get_weather_recommendation(location):
    ...
    try:
        ...
        print(f"An error occurred: {str(e)}")
def recommend_music(mood,corrected_location):
    ...
    client_id = 'db0eb6820fd74b50a79b3233e34164a8'
    client_secret = '0f77f5e9b8ba469f90b6f789da73bc96'
    ...
    if results and 'playlists' in results:
        ...
        if playlists:
            print(f"Here are some {mood} playlists you might like:")
...
def get_top_songs_by_location(corrected_location):
    ...
    query = f"top songs in {corrected_location}"
...
def main():
    ...
    # Use TextBlob to correct the location if it's misspelled
    corrected_location = str(TextBlob(location).correct())
    # Check if the corrected location is the same as the original input
    if corrected_location.lower() == location.lower():
        print(f"Location: {location}")
    else:
        print(f"Did you mean: {corrected_location}")
    # Get weather information and provide recommendations
    get_weather_info(corrected_location)
    # Ask if the user wants detailed weather information
    choice = input(f"Do you want detailed weather information for {corrected_location}? (yes/no): ").lower()
    if choice == 'yes':
        # Get detailed weather information and provide recommendations
        get_weather_recommendation(corrected_location)
    #else:
    #    # Ask the user about their mood
    #    mood = input("How are you feeling today (happy/upbeat/mellow/rainy/neutral)? ").lower()
    exit = True
    while exit:
        recom_choice = input("Would you like to know the type of music that is popular in your desired location? Or would you maybe prefer music based on your mood today?(location/mood)?[for exit type 'exit']")
            # Recommend music based on mood and location
        if recom_choice ==  'mood':
            mood = input("How are you feeling today (happy/upbeat/mellow/rainy/neutral)? ").lower()
            recommend_music(mood, corrected_location)
        elif recom_choice == 'location':
            top_songs, playlist_uri = get_top_songs_by_location(corrected_location)
            if top_songs:
                df = pd.DataFrame(top_songs)
                print(f"Top 50 Songs in {corrected_location}:")
                print(df)
        ...
        print(f"Have a great time in {corrected_location}!")
